...
QDataStream &operator<<(QDataStream & out, Reflector * item){
    out << item->type() << item->text() << item->textColor()
        << item->backgroundColor() << item->getStringPosition()
        << item->getOpticalDiameter() << item->getDivergence()
        << item->getReflectorDistance();
    return out;
}
QDataStream &operator>>(QDataStream & in, Reflector * & item){
    Reflector *tmp = dynamic_cast<Reflector *>(item);
    in >> tmp;
    QString text;
    QColor textColor;
    ...
    in >> text >> textColor >> backgroundColor >> stringPosition;
    item->setText(text);
...
static void saveItems(QList<QGraphicsItem *> items, QDataStream & out){
    for(QGraphicsItem *item : items){
        ...
        switch (item->type()) {       
        case Reflector::Type:
            out << dynamic_cast<Reflector *>(item);
            break;
...
static QList<QGraphicsItem *> readItems(QDataStream & in){
    ...
    while (!in.atEnd()) {
        ...
        case Reflector::Type:{
            Reflector *item = new Reflector(2.0, 2.0, 2.0);
            in >> item;
            items << item;
            break;
        }
